...
from wagtail.search.backends import get_search_backend, get_search_backends
...
def index(request):
    ...
    # sorted list to get the number of videos per page
    ENTRIES_PER_PAGE_CHOICES = sorted({10, 30, 60, 100, 250})

    # dict for choose ordering videos
    ORDERING_OPTIONS = {
        "-created_at": _("Newest"),
        "created_at": _("Oldest"),
        "title": _("Title: (A -> Z)"),
    ...
    entries_per_page = request.GET.get("entries_per_page", default_entries_per_page)

    # filter video by ordering
    videos = Video.objects.filter(collection__in=collections).order_by(ordering)
    ...
    if current_tag:
        try:
            videos = videos.filter(tags__name=current_tag)
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        response = render(
            ...
            "wagtailvideos/videos/index.html",
            {
                ...
                "current_tag": current_tag,  # for display current tags in page
                "collections": collections_to_choose,
                "current_ordering": ordering,  # for display current ordering in page
...
def edit(request, video_id):
    ...
    if not video._meta.get_field("file").storage.exists(video.file.name):
        ...
        messages.error(
            request,
            _("The source video file could not be found. Please change the source or delete the video.").format(
                video.title,
            ),
            buttons=[messages.button(reverse("wagtailvideos:delete", args=(video.id,)), _("Delete"))],
        )
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                buttons=[messages.button(reverse("wagtailvideos:edit", args=(video.id,)), _("Edit"))],
            )
            return redirect("wagtailvideos:index")
        else:
...
def usage(request, video_id):
    ...
    return render(request, "wagtailvideos/videos/usage.html", {"video": video, "used_by": page})
