...
class MediaFormats(ChoiceEnum):
    webm = _("VP8 and Vorbis in WebM")
    mp4 = _("H.264 and AAC in Mp4")
    ogg = _("Theora and Vorbis in Ogg")

    def get_quality_param(self, quality):
        if self is MediaFormats.webm:
            return {VideoQuality.lowest: "50", VideoQuality.default: "22", VideoQuality.highest: "4"}[quality]
        elif self is MediaFormats.mp4:
            return {VideoQuality.lowest: "28", VideoQuality.default: "24", VideoQuality.highest: "18"}[quality]
        elif self is MediaFormats.ogg:
            return {VideoQuality.lowest: "5", VideoQuality.default: "7", VideoQuality.highest: "9"}[quality]
...
class AbstractVideo(CollectionMember, index.Indexed, models.Model):
    ...
    thumbnail = models.ImageField(upload_to=get_upload_to, verbose_name=_("thumbnail"), null=True, blank=True)
    ...
    def video_tag(self, attrs=None):
        ...
        sources.append("<source src='{0}' type='{1}'>".format(self.url, self.content_type))
...
class TranscodingThread(threading.Thread):
    ...
    def run(self):
        ...
        transcode_name = "{0}.{1}".format(video.filename(include_ext=False), media_format.name)
        ...
        try:
            ...
            self.transcode.file = ContentFile(open(output_file, "rb").read(), transcode_name)
...
class AbstractVideoTranscode(models.Model):
    ...
    file = models.FileField(null=True, blank=True, verbose_name=_("file"), upload_to=get_upload_to)
...
class TrackListing(AbstractTrackListing):
    video = models.OneToOneField(Video, on_delete=models.CASCADE, related_name="track_listing")
